class Car {
    constructor(x, y, width, height, controlType, angle = 0, maxSpeed = 20, color = "blue") {
        ...
        this.reachedTarget = false;  // New property
    ...
    update(roadBorders, traffic, target) {
        if (!this.damaged && !this.reachedTarget) {  // Skip update if reached target
            ...
            const distanceToTarget = Math.hypot(this.x - target.center.x, this.y - target.center.y);
            if (distanceToTarget < 10) {  // Adjust threshold as needed
